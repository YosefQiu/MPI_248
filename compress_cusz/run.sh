...
# Usage: ./run.sh <input_image> <min_x> <min_y> <max_x> <max_y> <output_base_name> <ground_truth_img>
...
for ERROR_BOUND in 1E-1 1E-2 1E-3 1E-4 1E-5 1E-6 1E-7 1E-8 1E-9; do
    echo "======================================================================================="
    COMPRESSED_FILE=${OUTPUT_BASE_NAME}_rgb_channels_${ERROR_BOUND}.bin.cusza
    DECOMPRESSED_FILE_WITHOUT_CHANGE=${OUTPUT_BASE_NAME}_rgb_channels_${ERROR_BOUND}.bin.cuszx
    DECOMPRESSED_FILE=${OUTPUT_BASE_NAME}_rgb_channels_${ERROR_BOUND}.bin.cusza.out
    OUTPUT_IMAGE=${OUTPUT_BASE_NAME}_decompressed_${ERROR_BOUND}.png

    # Compress the binary file and capture compression time
    CMD="cusz -t f32 -m abs -e $ERROR_BOUND -i ./rgb_channels.bin -l $CHANNEL_LENGTH -z --report time"
    echo "[=== run === $CMD]"
    COMPRESSION_OUTPUT=$($CMD)
    COMPRESS_TIME=$(echo "$COMPRESSION_OUTPUT" | grep "(total)" | awk '{print $2}')
    echo "Total Compression Time: $COMPRESS_TIME"

    CMD="mv ./rgb_channels.bin.cusza $COMPRESSED_FILE"
    echo "[=== run === $CMD]"
    $CMD
    ...
    # Decompress the binary file and capture decompression time
    CMD="cusz -i $COMPRESSED_FILE -l $CHANNEL_LENGTH -x --report time"
    echo "[=== run === $CMD]"
    DECOMPRESSION_OUTPUT=$($CMD)
    DECOMPRESS_TIME=$(echo "$DECOMPRESSION_OUTPUT" | grep "(total)" | awk '{print $2}')
    echo "Total Decompression Time: $DECOMPRESS_TIME"

    CMD="mv $DECOMPRESSED_FILE_WITHOUT_CHANGE $DECOMPRESSED_FILE"
    echo "[=== run === $CMD]"
    $CMD
