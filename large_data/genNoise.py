...
# 设置分辨率
if len(sys.argv) != 2:
    print("Usage: python3 genNoise.py [resolution]")

resolution = int(sys.argv[1])

# 生成3D Perlin Noise数据集
def generate_perlin_noise(resolution, scale=10.0, octaves=1, persistence=0.5, lacunarity=2.0):
...
# 保存生成的数据集为.npy文件（可选）
npy_filename = f"perlin_noise_data_{resolution}.npy"
...
# 保存为 .raw 文件
raw_filename = f"perlin_noise_data_{resolution}.raw"
...
# 保存为 .vti 文件
vti_filename = f"perlin_noise_data_{resolution}.vti"
